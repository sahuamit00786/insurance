...
async function create(req, res) {
  ...
  if (String(password).length < 8) return error(res, 'Password must be at least 8 characters');
  ...
  // Send welcome email with set-password link (fire & forget — don't fail request if email fails)
  try {
    const token     = crypto.randomBytes(32).toString('hex');
    const expiresAt = new Date(Date.now() + 48 * 60 * 60 * 1000);
    await pool.query(
      'DELETE FROM password_resets WHERE user_id = ?', [result.insertId]
    );
    await pool.query(
    ...
    await sendMail({
      to:      email,
      subject: 'Welcome to InsurVault — Set your password',
      html:    welcomeEmailHtml({ name, resetUrl }),
    });
  } catch (emailErr) {
...
async function update(req, res) {
  ...
  if (req.body.password) {
    if (String(req.body.password).length < 8) return error(res, 'Password must be at least 8 characters');
    sets.push('password=?');
...
async function resetPassword(req, res) {
  ...
  const [existing] = await pool.query('SELECT id, role FROM users WHERE id = ? AND is_deleted = 0', [req.params.id]);
  if (!existing.length) return error(res, 'Staff not found', 404);
  if (existing[0].role === 'superadmin') return error(res, 'Cannot reset superadmin password', 403);

  await pool.query('UPDATE users SET password = ? WHERE id = ?', [await bcrypt.hash(password, 10), req.params.id]);
  return success(res, { message: 'Password reset successfully' });
}

module.exports = { list, getOne, create, update, remove, getPermissions, updatePermissions, resetPassword };
