...
async function getExpiry(req, res) {
  const { type = 'expiring', plan_code_id } = req.query;
  ...
  const extraWhere = plan_code_id ? ' AND i.plan_code_id = ?' : '';
  const extraParams = plan_code_id ? [plan_code_id] : [];
  ...
  const baseFrom = `
    ...
    WHERE i.premium_due_date IS NOT NULL AND i.is_deleted = 0 AND c.is_deleted = 0 AND ${dateFilter}${extraWhere}`;

  const [[{ total }]] = await pool.query(`SELECT COUNT(*) AS total ${baseFrom}`, extraParams);
